...
struct UpdateEditFormView: View {
  ...
  var body: some View {
    NavigationStack {
      Form {
        ...
        VStack {
          ...
          HStack {
            ...
            .alert(isPresented: .constant(cameraError != nil), error: cameraError) { error in
              Button("Ok") {
                if ((error.recoverySuggestion?.contains("Open Settings")) != nil) {
            ...
            .sheet(isPresented: $showCamera) {
              UIKitCamera(selectedImage: $vm.cameraImage).ignoresSafeArea()
            }
            PhotosPicker(selection: $imagePicker.imageSelection) {
              Label("Photos", systemImage: "photo")
            }
          }
          .foregroundStyle(.white).buttonStyle(.borderedProminent)
      ...
      .onChange(of: vm.cameraImage) { oldValue, newValue in
